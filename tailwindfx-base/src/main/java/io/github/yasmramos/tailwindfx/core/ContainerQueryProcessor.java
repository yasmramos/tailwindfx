 ...
 *
 * <p>Container queries allow styling elements based on the size of their container rather than the
 * viewport. This processor detects and handles container-related tokens.
 *
 * <p>Supported tokens:
 *
 * <ul>
 *   <li>{@code @[<breakpoint>]} - Container query breakpoints
 *   <li>{@code @min-*} - Minimum container size queries
 *   <li>{@code @max-*} - Maximum container size queries
 * </ul>
 *
 * <p>Note: JavaFX does not natively support container queries. This processor provides detection
 * and logging for future implementation or CSS export scenarios.
 *
 * @author yasmramos
 * @since 1.0.0
 */
public class ContainerQueryProcessor {
  ...
  /**
   * Centralized breakpoints from ThemeConfig (matching Tailwind CSS v4). sm: 640px, md: 768px, lg:
   * 1024px, xl: 1280px, 2xl: 1536px Using ConcurrentHashMap to allow dynamic additions while
   * maintaining thread safety.
   */
  private static final Map<String, Integer> BREAKPOINTS =
      new java.util.concurrent.ConcurrentHashMap<>(ThemeConfig.defaultConfig().breakpoints());
  ...
  public record ContainerQueryResult(
      boolean isContainerQuery, QueryType queryType, String value, String cssEquivalent) {}
