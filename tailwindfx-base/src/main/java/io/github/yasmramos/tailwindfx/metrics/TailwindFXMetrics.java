 ...
 *
 * <p>Tracks cache performance, method call counts, and timing data. All counters are thread-safe
 * ({@link AtomicLong}).
 *
...
public final class TailwindFXMetrics {
  ...
  public static TailwindFXMetrics instance() {
    return INSTANCE;
  }
  ...
  public void recordCacheHit() {
    if (enabled) jitCacheHits.incrementAndGet();
  }
  ...
  public void recordCompilation() {
    if (enabled) jitCompilations.incrementAndGet();
  }
  ...
  public void recordConflictResolution(String category) {
    ...
    conflictsByCategory.computeIfAbsent(category, k -> new AtomicLong()).incrementAndGet();
  }

  /** Records a theme switch (preset change or dark/light toggle). */
  public void recordThemeSwitch() {
    if (enabled) themeSwitches.incrementAndGet();
  }

  /** Records an animation start. */
  public void recordAnimationPlay() {
    if (enabled) animationPlays.incrementAndGet();
  }
  ...
  public long layoutPasses() {
    return layoutPasses.get();
  }

  /**
   * Average TwFlexPane layout time in nanoseconds. Returns {@code 0} if no layout passes have been
   * recorded.
   */
  ...
  public long conflictResolutions() {
    return conflictResolutions.get();
  }

  /** Total theme switches since last reset. */
  public long themeSwitches() {
    return themeSwitches.get();
  }

  /** Total animation plays since last reset. */
  public long animationPlays() {
    return animationPlays.get();
  }

  /**
   * JIT cache hit ratio as a fraction [0.0, 1.0]. Returns {@code 1.0} if no lookups have been made
   * yet.
   */
  ...
  /**
   * Average compilation time per token in nanoseconds. Returns {@code 0} if no timed compilations
   * have been recorded.
   */
  ...
  private void checkAlerts() {
    ...
    if (minCacheHitRatio > 0 && total > 100) {
      ...
      if (ratio < minCacheHitRatio) alertHandler.onAlert("cacheHitRatio", ratio, minCacheHitRatio);
    }
    if (maxConflictRate > 0 && applyCalls.get() > 0) {
      double rate = (double) conflictResolutions.get() / applyCalls.get();
      if (rate > maxConflictRate) alertHandler.onAlert("conflictRate", rate, maxConflictRate);
    }
    if (maxAvgCompileMs > 0 && totalCompileCount.get() > 0) {
      double avgMs = avgCompileNs() / 1_000_000.0;
      if (avgMs > maxAvgCompileMs) alertHandler.onAlert("avgCompileMs", avgMs, maxAvgCompileMs);
    }
  }
  ...
  public String report() {
    ...
    String hitPct = (hits + misses) == 0 ? "N/A" : String.format("%.1f%%", cacheHitRatio() * 100);
    String avgMs =
        totalCompileCount.get() == 0
            ? "N/A"
    ...
    if (layoutPasses.get() > 0) {
      ...
      sb.append(
          String.format("  Layout passes:        %,8d  (avg %s)%n", layoutPasses.get(), avgLayout));
    }
    return sb.toString();
  }
   ...
   *
   * <ul>
   *   <li><b>Low JIT cache hit ratio</b> (&lt;50% after 100+ lookups) — possible token explosion
   *   <li><b>High conflict resolution rate</b> (&gt;30% of apply calls) — excessive class churn
   *   <li><b>Slow layout passes</b> (&gt;2ms average) — TwFlexPane/TwGridPane overload
   *   <li><b>Slow JIT compilation</b> (&gt;1ms average) — complex token patterns
   *   <li><b>Metrics not enabled</b> — no data collected yet
   * </ul>
   *
   * <pre>
    ...
    if (applyCalls() > 0 && conflictResolutions() > applyCalls() * 0.30) {
      issues.add(
          new HealthIssue(
              "WARN",
              String.format(
                  "High conflict resolution rate: %d conflicts / %d applies (%.0f%%)",
                  conflictResolutions(),
                  applyCalls(),
    ...
    if (layoutPasses() >= 10 && avgLayoutNs() > 2_000_000) {
      issues.add(
          new HealthIssue(
              "WARN",
              String.format(
                  "Slow TwFlexPane/TwGridPane layout: avg %.2f ms over %d passes",
    ...
    if (compilations() >= 10 && avgCompileNs() > 1_000_000) {
      issues.add(
          new HealthIssue(
              "WARN",
              String.format(
                  "Slow JIT compilation: avg %.2f ms over %d compilations",
