 ...
 *
 * <p>This processor handles arbitrary values with explicit type hints to disambiguate utility
 * classes that could accept multiple types of values.
 *
 * <p>Supported type hints:
 *
 * <ul>
 *   <li>{@code length} - for lengths (px, rem, em, etc.)
 *   <li>{@code percentage} - for percentages (%)
 *   <li>{@code number} - for unitless numbers
 *   <li>{@code color} - for colors (hex, rgb, rgba, named colors)
 *   <li>{@code angle} - for angles (deg, rad, turn)
 *   <li>{@code url} - for URLs
 *   <li>{@code image} - for image functions
 *   <li>{@code family-name} - for font families
 *   <li>{@code line-width} - for border widths
 *   <li>{@code shape} - for shape functions
 *   <li>{@code position} - for background positions
 *   <li>{@code bg-size} - for background sizes
 * </ul>
 *
 * <p>Examples:
 *
 * <ul>
 *   <li>{@code w-[length:320px]} → width: 320px
 *   <li>{@code text-[percentage:50%]} → opacity: 0.5
 *   <li>{@code rotate-[angle:45deg]} → rotate: 45deg
 *   <li>{@code bg-[color:#ff0000]} → background-color: #ff0000
 *   <li>{@code opacity-[number:0.5]} → opacity: 0.5
 *   <li>{@code font-['Custom_Font']} → font-family: 'Custom Font'
 * </ul>
 *
 * @author yasmramos
 * @since 1.0.0
...
public final class TypeHintProcessor {
   ...
   */
  public record TypeHintResult(TypeHint typeHint, String value, String originalValue) {
    /**
