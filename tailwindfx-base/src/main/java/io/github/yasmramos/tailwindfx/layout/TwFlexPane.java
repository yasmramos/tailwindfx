 ...
 *
 * <p>Fills the gap between JavaFX's existing layout panes and the CSS flexbox model. No existing
 * JavaFX pane supports wrapping + alignment + gap simultaneously. This pane implements all three.
 *
 * <h3>What this adds over existing panes</h3>
 *
 * <ul>
 *   <li>{@code HBox} — row only, no wrap, no align-items control.
 *   <li>{@code VBox} — column only, no wrap.
 *   <li>{@code FlowPane} — wraps but has no justify-content or align-items.
 *   <li>{@code TilePane} — forces equal size on all children.
 *   <li><b>TwFlexPane</b> — direction, wrap, justify, align, gap — all at once.
 * </ul>
 *
 * <h3>Usage</h3>
 *
...
public class TwFlexPane extends Pane {
  ...
  /**
   * Main axis distribution (analogous to {@code justify-content}). Controls how children are spaced
   * along the main axis.
   */
  public enum Justify {
    START,
  ...
  /**
   * Cross axis alignment (analogous to {@code align-items}). Controls how children are aligned
  ...
  /**
   * Multi-line alignment (analogous to {@code align-content}). Controls how multiple lines are
   * distributed in the cross-axis when there's extra space. Only applies when wrap=true and there
   * are multiple lines.
   */
  ...
  /** Node property key for per-child align override (analogous to {@code align-self}) */
  private static final String SELF_KEY = "tailwindfx.flex.align-self";

  /**
   * Internal cache to avoid repeated map lookups and autoboxing during layout. Uses WeakHashMap to
   * prevent memory leaks.
   */
  private static final java.util.WeakHashMap<Node, FlexData> flexDataCache =
  ...
  /**
   * Gets cached flex data for a node, updating from properties if needed. This reduces autoboxing
  ...
  /**
   * Sets align-content (multi-line alignment). Triggers layout. Only applies when wrap=true and
   ...
   *
   * <p>Fades all children out, switches direction, then fades back in. Useful for responsive
   * breakpoint switches where an abrupt re-layout looks jarring.
   *
   * <pre>
   ...
   * @param durationMs fade duration in milliseconds (each way); use 0 for instant
   */
  public void setDirectionAnimated(Direction d, int durationMs) {
    ...
    javafx.animation.Timeline fadeOut =
          ...
          javafx.animation.Timeline fadeIn =
              new javafx.animation.Timeline(
                  new javafx.animation.KeyFrame(
                      javafx.util.Duration.millis(durationMs),
                      new javafx.animation.KeyValue(
                          opacityProperty(), 1.0, javafx.animation.Interpolator.EASE_OUT)));
          fadeIn.play();
        });
    fadeOut.play();
  }
  ...
  // Per-child flex-grow
  /**
   * Sets the grow factor for a child node (analogous to {@code flex-grow}). A grow factor of 1
   * means the child takes available space; 0 (default) means it does not grow.
   *
   ...
   *
   * <p>May be called before or after adding the node to a {@link TwFlexPane}. If called before, a
   * listener is installed to trigger layout as soon as the node is added to its parent.
   *
   * @param node the child node
  ...
  public static void setGrow(Node node, double factor) {
    ...
    if (factor < 0) {
      throw new IllegalArgumentException("TwFlexPane.setGrow: factor must be >= 0, got: " + factor);
    }
  ...
  /**
   * Installs a one-shot {@code parentProperty} listener that calls {@code requestLayout()} on the
   * parent when the node is first added to a {@link TwFlexPane}. This ensures flex properties
   * applied before a node is added to its parent take effect immediately.
   *
   * <p>The listener removes itself after the first TwFlexPane parent is detected, so it does not
   * accumulate.
   */
  private static void ensureLayoutOnParent(Node node) {
    ...
    javafx.beans.value.ChangeListener<Parent> listener =
        new javafx.beans.value.ChangeListener<Parent>() {
          @Override
          public void changed(
              javafx.beans.value.ObservableValue<? extends Parent> obs, Parent oldP, Parent newP) {
  ...
  /**
   * Sets the shrink factor for a child node (analogous to {@code flex-shrink}).
   *
   * <p>A shrink factor > 0 means the child will shrink proportionally when there is not enough
   * space. The default is 1 (shrinks if needed). Set to 0 to prevent shrinking (analogous to {@code
   * flex-shrink: 0}).
   *
   * @param node the child node
  ...
  /**
   * Sets the visual display order for a child node (analogous to CSS {@code order}).
   *
   * <p>Children are sorted by order value before layout. Lower values appear first. The default
   * order is 0. Negative values are allowed and place the child before all order-0 items.
   *
   * <pre>
  ...
  /**
   * Sets a per-child alignment override on the cross axis (analogous to {@code align-self}).
   *
   * <p>Overrides the container's {@link #setAlign(Align)} for this specific child. Set to {@code
   * null} to use the container's default.
   *
   * <pre>
   ...
   *
   * @param node the child node
   * @param align cross-axis alignment for this child, or {@code null} to inherit
   */
  ...
  /** Returns the per-child align-self, or {@code null} if inheriting from the container. */
  ...
  /**
   * Sets the initial main-axis size for a child before flex-grow and flex-shrink are applied
   * (analogous to CSS {@code flex-basis}).
   *
   * <p>In row direction, this overrides the child's preferred <em>width</em>. In column direction,
   * this overrides the child's preferred <em>height</em>.
   *
   * <p>Special values:
   *
   * <ul>
   *   <li>{@code -1} — use the child's natural preferred size (default, same as {@code flex-basis:
   *       auto})
   *   <li>{@code 0} — start from zero before distribution ({@code flex-basis: 0})
   *   <li>{@code > 0} — fixed base size in px before grow/shrink
   * </ul>
   *
   * <pre>
  ...
  /**
   * Returns the flex-basis for a child, or {@code -1} if using natural pref size.
   *
   * @param node the child node
  ...
  // Layout math helpers
  /**
   * Computes starting positions for items along one axis given justify-content.
   *
  ...
  static double[] justifyPositions(double[] sizes, double gap, double total, Justify j) {
    ...
    switch (j) {
      ...
      case BETWEEN -> {
        ...
        double spaceBetween =
            sizes.length > 1 ? Math.max(gap, (total - usedSpace) / (sizes.length - 1)) : gap;
        double x = 0;
        for (int i = 0; i < sizes.length; i++) {
          pos[i] = x;
  ...
  /**
   * Computes starting Y positions for rows/lines when wrap=true based on align-content. Similar to
   * justifyPositions but for the cross-axis distribution of multiple lines.
   *
   ...
   */
  static double[] alignContentPositions(
      double[] rowHeights, double gap, double total, AlignContent ac) {
    double usedSpace = 0;
    ...
    switch (ac) {
      ...
      case BETWEEN -> {
        ...
        double spaceBetween =
            rowHeights.length > 1
                ? Math.max(gap, (total - usedSpace) / (rowHeights.length - 1))
                : gap;
        double y = 0;
        for (int i = 0; i < rowHeights.length; i++) {
          pos[i] = y;
  ...
  /**
   * Returns the main-axis base size for a child, respecting flex-basis. Used in row-direction
   * layouts.
   */
  ...
  /**
   * Returns the cross-axis base size for a child, respecting flex-basis. Used in column-direction
   * layouts.
   */
  ...
  /**
   * Returns the natural preferred width (ignoring flex-basis). Used for cross-axis size in column
   * layouts.
   */
  ...
  /**
   * Returns the natural preferred height (ignoring flex-basis). Used for cross-axis size in row
   * layouts.
   */
