...
class TwGridPaneTest extends ApplicationTest {
  ...
  void testBuilderInvalidCols() {
    assertThrows(IllegalArgumentException.class, () -> TwGridPane.create().cols(0).build());

    assertThrows(IllegalArgumentException.class, () -> TwGridPane.create().cols(-1).build());
  }
  ...
  void testBuilderInvalidRows() {
    assertThrows(IllegalArgumentException.class, () -> TwGridPane.create().rows(-1).build());
  }
  ...
  void testBuilderSeparateGaps() {
    TwGridPane grid = TwGridPane.create().gapX(8.0).gapY(16.0).build();
  ...
  void testBuilderAutoFlow() {
    TwGridPane grid = TwGridPane.create().autoFlow(TwGridPane.AutoFlow.COL).build();

    assertEquals(TwGridPane.AutoFlow.COL, grid.getAutoFlow());

    grid = TwGridPane.create().autoFlow(TwGridPane.AutoFlow.ROW_DENSE).build();
  ...
  void testSetInvalidColSpan() {
    ...
    assertThrows(IllegalArgumentException.class, () -> TwGridPane.setColSpan(child, 0));

    assertThrows(IllegalArgumentException.class, () -> TwGridPane.setColSpan(child, -1));
  }

  @Test
  @DisplayName("Should throw exception when setting col span on null node")
  void testSetColSpanOnNullNode() {
    assertThrows(NullPointerException.class, () -> TwGridPane.setColSpan(null, 2));
  }
  ...
  void testSetInvalidRowSpan() {
    ...
    assertThrows(IllegalArgumentException.class, () -> TwGridPane.setRowSpan(child, 0));
  }

  @Test
  @DisplayName("Should throw exception when setting row span on null node")
  void testSetRowSpanOnNullNode() {
    assertThrows(NullPointerException.class, () -> TwGridPane.setRowSpan(null, 2));
  }

  @Test
  @DisplayName("Should place child in template area")
  void testPlaceInArea() {
    TwGridPane grid = TwGridPane.create().areas("header header", "sidebar main").build();
  ...
  void testPlaceInNonExistentArea() {
    TwGridPane grid = TwGridPane.create().areas("header main").build();

    Label child = new Label("Test");

    assertThrows(IllegalArgumentException.class, () -> grid.placeIn(child, "nonexistent"));
  }
  ...
  void testPlaceInNullNode() {
    ...
    assertThrows(IllegalArgumentException.class, () -> grid.placeIn(null, "header"));
  }
