...
public class BenchmarkTest {
  ...
  void testCacheHitFasterThanMiss() {
    ...
    assertTrue(results.getSpeedupFactor() > 1.0, "Cache hits should be faster than cache misses");

    // Typically cache should be at least 2x faster (relaxed for CI environments)
    assertTrue(
        results.getSpeedupFactor() > 2.0,
        "Cache speedup should be at least 2x in normal conditions, got: "
  ...
  void testThroughput() {
    ...
    assertTrue(
        results.throughput().value() > 10_000, "Throughput should exceed 10,000 compilations/sec");
  ...
  void testCacheMissTime() {
    ...
    assertTrue(results.cacheMiss().value() > 0.001, "Cache miss time should be measurable");

    // But shouldn't be too slow (< 10ms per compilation)
    assertTrue(results.cacheMiss().value() < 10.0, "Cache miss should complete in under 10ms");
  }
  ...
  void testCacheHitTime() {
    ...
    assertTrue(results.cacheHit().value() < 0.1, "Cache hit should complete in under 0.1ms");

    // And measurable (> 0)
    assertTrue(results.cacheHit().value() > 0, "Cache hit time should be positive");
  }
  ...
  void testMixedWorkload() {
    ...
    assertTrue(mixed >= hit * 0.5, "Mixed workload time should be at least half of hit time");

    // Log for debugging
    System.out.println(
        String.format("Hit: %.4f ms, Miss: %.4f ms, Mixed: %.4f ms", hit, miss, mixed));
  }
