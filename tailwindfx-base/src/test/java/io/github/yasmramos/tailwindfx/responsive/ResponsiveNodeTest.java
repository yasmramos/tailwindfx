...
/** Tests for {@link ResponsiveNode} — per-node responsive rules driven by Scene width. */
public final class ResponsiveNodeTest {
  ...
  static void testNullGuards() {
    throws_("on(null)", IllegalArgumentException.class, () -> ResponsiveNode.on(null));
    throws_(
        "at(-1, classes)",
  ...
  static void testBaseAppliedImmediately() throws Exception {
    runFx(
        () -> {
          ...
          ResponsiveNode rn = ResponsiveNode.on(node).base("flex-col", "p-4").install(scene(300));
  ...
  static void testBaseAndBreakpoint() throws Exception {
    runFx(
        () -> {
          ...
          ResponsiveNode rn = ResponsiveNode.on(node).base("w-full").md("w-48").install(scene(800));
  ...
  static void testBreakpointReplacesPrevious() throws Exception {
    ...
    runFx(
        () -> {
          ...
          sceneRef.set(sc);
          ResponsiveNode rn = ResponsiveNode.on(node).sm("w-full").md("w-48").install(sc);
          rnRef.set(rn);
        });
  ...
  static void testOnBreakpointNullCallback() throws Exception {
    runFx(
        () -> {
          ...
          try {
            ResponsiveNode rn =
                ResponsiveNode.on(node).md("w-48").onBreakpoint(null).install(scene(800));
  ...
  static void testMultipleBreakpoints() throws Exception {
    runFx(
        () -> {
          ...
          ResponsiveNode rn =
              ResponsiveNode.on(node).sm("w-full").md("w-2/3").lg("w-1/2").install(scene(1100));
  ...
  static void testAutoDetachOnSceneRemoval() throws Exception {
    runFx(
        () -> {
          ...
          ResponsiveNode rn = ResponsiveNode.on(node).sm("w-full").install(sc);
          check(
              "listener marker installed",
  ...
  static void testRefresh() throws Exception {
    runFx(
        () -> {
          Region node = new Region();
          ResponsiveNode rn =
              ResponsiveNode.on(node).base("w-full").md("w-48").install(scene(900)); // MD active
  ...
  static void testActiveBreakpointValue() throws Exception {
    runFx(
        () -> {
          ...
          ResponsiveNode rn = ResponsiveNode.on(node).md("w-48").lg("w-32").install(scene(900));
          ...
          ResponsiveNode rn2 = ResponsiveNode.on(node2).md("w-48").lg("w-32").install(scene(1200));
  ...
  static void testOrderIndependent() throws Exception {
    runFx(
        () -> {
          ...
          Region node = new Region();
          ResponsiveNode rn =
              ResponsiveNode.on(node).lg("w-32").sm("w-full").md("w-48").install(scene(900));
          check("md(768) wins at 900, not lg(1024)", node.getStyleClass().contains("w-48"));
          check("w-32 not applied at 900", !node.getStyleClass().contains("w-32"));
          rn.detach();
        });
  }
}
