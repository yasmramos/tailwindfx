...
class JitCompilerTest {
  ...
  class GradientSupportTests {
    ...
    void testGradientDirectionBottom() {
      JitCompiler.BatchResult result =
          JitCompiler.compileBatch("bg-gradient-to-b", "from-blue-500", "to-purple-600");
    ...
    void testGradientDirectionRight() {
      JitCompiler.BatchResult result =
          JitCompiler.compileBatch("bg-gradient-to-r", "from-gray-800", "to-gray-900");
    ...
    void testGradientBottomRight() {
      JitCompiler.BatchResult result =
          JitCompiler.compileBatch("bg-gradient-to-br", "from-blue-500", "to-purple-600");
    ...
    void testAllGradientDirections() {
      ...
      for (int i = 0; i < directions.length; i++) {
        JitCompiler.BatchResult result =
            JitCompiler.compileBatch(directions[i], "from-blue-500", "to-purple-600");

        assertTrue(
            result.hasInlineStyle(), "Gradient " + directions[i] + " should produce inline style");
        assertTrue(
            result.inlineStyle().contains(expectedDirs[i]),
    ...
    void testGradientDefaultColors() {
      ...
      JitCompiler.BatchResult result = JitCompiler.compileBatch("bg-gradient-to-b");
    ...
    void testGradientWithOnlyFromColor() {
      JitCompiler.BatchResult result =
          JitCompiler.compileBatch("bg-gradient-to-b", "from-blue-500");
    ...
    void testGradientWithOnlyToColor() {
      JitCompiler.BatchResult result =
          JitCompiler.compileBatch("bg-gradient-to-b", "to-purple-600");
  ...
  class BatchTests {
    ...
    void testSkipNullAndBlank() {
      JitCompiler.BatchResult result = JitCompiler.compileBatch(null, "", "  ", "p-4");
    ...
    void testTokensWithSpaces() {
      JitCompiler.BatchResult result = JitCompiler.compileBatch("bg-white p-4 rounded-lg");
  ...
  class RingTests {
    ...
    void testColoredRing() {
      JitCompiler.BatchResult result = JitCompiler.compileBatch("ring-2", "ring-blue-500");
