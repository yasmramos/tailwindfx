...
/**
 * Tests for {@link BreakpointManager} — breakpoint detection, callbacks, custom breakpoints,
 * orientation, and detach.
 */
public final class BreakpointManagerTest {
  ...
  static void testForWidth() {
    ...
    eq("320 → XS", BreakpointManager.Breakpoint.XS, BreakpointManager.Breakpoint.forWidth(320));
    eq("640 → SM", BreakpointManager.Breakpoint.SM, BreakpointManager.Breakpoint.forWidth(640));
    eq("768 → MD", BreakpointManager.Breakpoint.MD, BreakpointManager.Breakpoint.forWidth(768));
    eq("1024 → LG", BreakpointManager.Breakpoint.LG, BreakpointManager.Breakpoint.forWidth(1024));
    eq("1280 → XL", BreakpointManager.Breakpoint.XL, BreakpointManager.Breakpoint.forWidth(1280));
    eq("1536 → XXL", BreakpointManager.Breakpoint.XXL, BreakpointManager.Breakpoint.forWidth(1536));
  }

  static void testForWidthExact() {
    // Exact boundary: 639 = XS, 640 = SM
    eq("639 → XS", BreakpointManager.Breakpoint.XS, BreakpointManager.Breakpoint.forWidth(639));
    eq("640 → SM", BreakpointManager.Breakpoint.SM, BreakpointManager.Breakpoint.forWidth(640));
    eq("767 → XS or SM", true, BreakpointManager.Breakpoint.forWidth(767).minWidth < 768);
    eq(
        "768 → MD exactly",
        BreakpointManager.Breakpoint.MD,
        BreakpointManager.Breakpoint.forWidth(768));
  }

  static void testForWidthZero() {
    eq("0 → XS", BreakpointManager.Breakpoint.XS, BreakpointManager.Breakpoint.forWidth(0));
    eq("-1 → XS", BreakpointManager.Breakpoint.XS, BreakpointManager.Breakpoint.forWidth(-1));
  }
  ...
  static void testBreakpointEnum() {
    ...
    for (int i = 1; i < bps.length; i++) {
      check("ordered: " + bps[i - 1] + " < " + bps[i], bps[i - 1].minWidth < bps[i].minWidth);
    }
    // Each has a non-blank CSS class
    for (BreakpointManager.Breakpoint bp : bps) {
      check(bp + ".cssClass not blank", bp.cssClass != null && !bp.cssClass.isBlank());
    }
  }
  ...
  static void testCssClassInjected() throws Exception {
    runFx(
        () -> {
          ...
          boolean hasAnyBpClass = root.getStyleClass().stream().anyMatch(c -> c.startsWith("bp-"));
