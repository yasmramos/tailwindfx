  ...
  static void testScopeCopy() throws Exception {
    runFx(
        () -> {
          ...
          check("copy: target has accent", target.getStyle().contains("-fx-accent"));
        });
  }
  ...
  static void testScopePresetDark() throws Exception {
    runFx(
        () -> {
          ...
          check("scope dark: has -fx-base", pane.getStyle().contains("-fx-base"));
          check("scope dark: dark class", pane.getStyleClass().contains("dark"));
        });
  }
  ...
  static void testFindClosestScope() throws Exception {
    runFx(
        () -> {
          ...
          // No scope yet
          check("no scope: findClosest=null", ThemeScopeManager.findClosestScope(child) == null);
