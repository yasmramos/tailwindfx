...
class VariantParserTest {
  ...
  void testToCssSelector_arbitraryVariant() {
    assertEquals(
        "@media(min-width:768px)", VariantParser.toCssSelector("[@media(min-width:768px)]"));
    assertEquals("&:hover", VariantParser.toCssSelector("[&:hover]"));
  }
  ...
  void testParseComplexChainedVariants() {
    VariantParser.VariantResult result =
        VariantParser.parse("group-hover:md:hover:dark:bg-blue-500");
