...
/** Tests for {@link TwDataTable} — builder, sorting, filtering, pagination, search. */
    ...
    runFx(
        () -> {
          ...
          boolean hasTextField =
              t.container().getChildren().stream()
                  .anyMatch(
                      n ->
  ...
  static void testContainerAccess() throws Exception {
    runFx(
        () -> {
          ...
          check("container() non-null", t.container() != null);
          check(
              "container has table as child", t.container().getChildren().contains(t.tableView()));
        });
  }
        ...
        () -> {
          TwDataTable<Person> t =
              TwDataTable.of(Person.class)
                  ...
                  .build();
          check("table-striped applied", t.tableView().getStyleClass().contains("table-striped"));
          check("table-hover applied", t.tableView().getStyleClass().contains("table-hover"));
          check("base table-view applied", t.tableView().getStyleClass().contains("table-view"));
        });
  }
