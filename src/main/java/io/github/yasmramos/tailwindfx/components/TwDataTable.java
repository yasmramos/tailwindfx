 ...
 *
 * <ul>
 *   <li>{@code .table-view} — base table style (from tailwindfx.css)
 *   <li>{@code .table-striped} — alternating row colors
 *   <li>{@code .table-hover} — hover highlight on rows
 *   <li>{@code .table-compact} — reduced row height
 *   <li>{@code .table-bordered}— cell borders
 * </ul>
 *
...
public final class TwDataTable<T> {
  ...
  public static final class Builder<T> {
    ...
    public Builder<T> searchable(boolean on) {
      this.searchable = on;
      return this;
    }
    ...
    public Builder<T> pageSize(int size) {
      if (size < 1)
        throw new IllegalArgumentException("TwDataTable.pageSize: must be >= 1, got: " + size);
      this.paginated = true;
      this.pageSize = size;
      return this;
    }
    ...
    public Builder<T> placeholder(String text) {
      this.placeholder = text;
      return this;
    }

    /** Applies additional TailwindFX CSS classes to the TableView. */
    public Builder<T> style(String... classes) {
  ...
  /**
   * Sets the data items displayed in the table. Replaces all existing items.
   *
  ...
  /** Returns the current page index (0-based). Always returns 0 if pagination is disabled. */
  public int currentPage() {
    return currentPage;
  }

  /** Returns the total number of pages. Returns 1 if pagination is disabled. */
  ...
  private void buildColumns(List<ColDef<T>> defs) {
    for (ColDef<T> def : defs) {
      ...
      col.setCellValueFactory(
          data -> new ReadOnlyObjectWrapper<>(def.value().apply(data.getValue())));
  ...
  private TableRow<T> buildStyledRow() {
    return new TableRow<>() {
      ...
      protected void updateItem(T item, boolean empty) {
        ...
        if (!empty && item != null) {
          if (getIndex() % 2 == 0) getStyleClass().add("row-even");
          else getStyleClass().add("row-odd");
        }
      }
    };
  }
  ...
  private void updatePaginationLabel(Label label, Button prev, Button next) {
    int total = pageCount();
    label.setText("Page " + (currentPage + 1) + " of " + total + " (" + filteredSize() + " items)");
