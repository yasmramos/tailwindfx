 ...
 *
 * <p>Wraps {@link GridPane} with a fluent API covering features that neither plain {@code GridPane}
 * nor CSS utility classes can express directly:
 *
 * <ul>
 *   <li><b>grid-auto-flow</b> — {@link #autoFlow(AutoFlow)}: children placed automatically in row
 *       or column order, with optional dense packing.
 *   <li><b>grid-template-areas</b> — {@link #areas(String...)}: named regions ({@code "header
 *       header"}, {@code "sidebar main"}) mapped to children by name via {@link #placeIn(Node,
 *       String)}.
 *   <li><b>grid-cols / grid-rows utilities</b> — {@link #cols(int)}, {@link #rows(int)}: uniform
 *       column/row count with equal sizing.
 *   <li><b>Masonry-like layout</b> — {@link #masonry(int)}: auto-flow column mode with items placed
 *       in the shortest column.
 *   <li><b>gap semantics</b> — {@link #gap(double)}, {@link #gapX(double)}, {@link #gapY(double)}:
 *       consistent with TwFlexPane.
 * </ul>
 *
 * <h3>Usage</h3>
 *
...
public final class TwGridPane extends Pane {
  ...
  // Enums
  /**
   * Auto-flow direction for automatically placed children (analogous to CSS {@code
   * grid-auto-flow}).
   */
  ...
  // Template areas
  /**
   * Parses area template strings and builds the area map. Each string is a row; each word is a cell
   * name. Consecutive identical names in the same row = column span. Identical names in consecutive
   * rows at same column = row span.
   *
   * <pre>
   * grid.areas(
  ...
  private void parseAreaTemplates(String[] templates) {
    ...
    for (int r = 0; r < grid.length; r++) {
      for (int c = 0; c < grid[r].length; c++) {
        ...
        // Measure rowSpan
        int rs = 1;
        while (r + rs < grid.length && grid[r + rs].length > c && grid[r + rs][c].equals(name)) {
          rs++;
        }
   ...
   *
   * @param templates row strings (e.g. {@code "header header"}, {@code "sidebar main"})
   */
  ...
  private void layoutAutoFlow(List<Node> children, double w, double h, double ox, double oy) {
    ...
    double cellH =
        gridRows > 1 ? (h - gapY * (gridRows - 1)) / gridRows : computeMaxCellHeight(children);
  ...
  private boolean canPlace(
      boolean[][] occupied, int c, int r, int cs, int rs, int maxCols, int maxRows) {
  ...
  private double computeMaxCellHeight(List<Node> children) {
    return children.stream()
        .mapToDouble(n -> n instanceof Region r ? r.prefHeight(-1) : n.prefHeight(-1))
        .max()
        .orElse(48);
  }
