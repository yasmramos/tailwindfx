...
/**
 * TwI18n — Internationalization helper that dynamically updates UI text when the locale changes
 * without requiring application restart.
 *
 * <p>Links JavaFX nodes with ResourceBundle keys and automatically rebinds their text properties
 * when the active locale changes. Supports:
 *
 * <ul>
 *   <li>Automatic binding to Label, Button, TextField prompts, Tooltips, etc.
 *   <li>Parameterized messages with MessageFormat
 *   <li>Global locale switching with instant UI updates
 *   <li>Fallback to key name if translation is missing
 *   <li>Custom ResourceBundle base names
 * </ul>
 *
 * <h3>Setup</h3>
 *
 ...
 *
 * <ul>
 *   <li>Label, Button, CheckBox, RadioButton (text)
 *   <li>TextField, TextArea (promptText)
 *   <li>Tooltip (text)
 *   <li>TableColumn (text)
 *   <li>Tab (text)
 *   <li>MenuItem (text)
...
public final class TwI18n {
  ...
  /**
   * Sets the base name for ResourceBundle lookup. E.g., "i18n/messages" will look for
   * i18n/messages_en.properties, etc.
   *
  ...
  /**
   * Gets a translated string for the given key in the current locale. Returns the key itself if no
  ...
  /**
   * Gets a translated string with MessageFormat parameters. E.g., get("welcome.user", "John") where
   * properties has "welcome.user=Hello {0}!"
   *
   * @param key the translation key
  ...
  /**
   * Creates a StringBinding with MessageFormat parameters. The parameters supplier is called each
   * time the binding updates.
   *
   * @param key the translation key
  ...
  /**
   * Binds a node's text property to a translation key. Automatically detects the node type and
   * binds the appropriate property.
   *
  ...
  /** Unbinds all active string bindings. Useful when completely resetting the i18n system. */
  public static void clearBindings() {
    activeBindings.forEach(StringBinding::dispose);
    activeBindings.clear();
  }

  /** Clears the ResourceBundle cache. Forces reloading of bundles on next locale switch. */
