 ...
 *
 * <p>JavaFX does not have @media queries. This engine simulates them: 1. Listens to
 * Stage.widthProperty() (and optionally heightProperty) 2. Calculates the active breakpoint 3.
 * Injects cumulative CSS classes into the Scene root (.bp-sm, .bp-md...) 4. Executes registered
 * callbacks when crossing each breakpoint
 *
 * <p>Default breakpoints (same as Tailwind): XS < 640px → no class SM ≥ 640px → .bp-sm MD ≥ 768px →
 * .bp-sm .bp-md LG ≥ 1024px → .bp-sm .bp-md .bp-lg XL ≥ 1280px → .bp-sm .bp-md .bp-lg .bp-xl 2XL ≥
 * 1536px → .bp-sm .bp-md .bp-lg .bp-xl .bp-2xl
 *
 * <p>Classes are CUMULATIVE like in Tailwind. .bp-lg implies that .bp-sm and .bp-md are also
 * active.
 *
 * <p>Basic usage: TailwindFX.responsive(stage) .onBreakpoint(Breakpoint.MD, () -> adaptLayout());
 *
 * <p>With custom breakpoints: BreakpointManager.custom() .add("phone", 0) .add("tablet", 600)
 * .add("desktop", 960) .attach(stage);
 *
 * <p>CSS in tailwindfx.css: .bp-md .sidebar { -fx-pref-width: 240px; } .bp-sm .sidebar {
 * -fx-pref-width: 60px; } .bp-lg .card-grid { -fx-vgap: 20px; }
 *
 * <p>Architecture: Publisher pattern - Single listener per Stage with throttling (~120ms) - Exposes
 * activeBreakpointProperty() for subscribers (e.g., VariantManager) - Prevents O(N) listeners by
 * centralizing breakpoint detection
 */
public final class BreakpointManager {

  // Cache of BreakpointManager instances per Stage
  private static final ConcurrentHashMap<Stage, BreakpointManager> instances =
  ...
  public enum Breakpoint {
    ...
    public static Breakpoint forWidth(double width) {
      ...
      for (Breakpoint bp : values()) if (width >= bp.minWidth) current = bp;
      return current;
    }
  }
  ...
  /**
   * Shorthand alias for {@link Breakpoint} — allows the concise form {@code
   * BreakpointManager.BP.MD} instead of {@code BreakpointManager.Breakpoint.MD}.
   *
   * <pre>
  ...
  /**
   * Gets or creates a BreakpointManager instance for the given Stage. Uses a cache to ensure only
  ...
  public static BreakpointManager attach(Stage stage) {
  ...
  /**
   * Registers a callback that executes when crossing a breakpoint (both up and down). Prevents
   * duplicates: if the same callback is already registered for this breakpoint, it won't be added
   * again.
   */
   ...
   * active=true → crossed upward (larger screen) active=false → crossed downward (smaller screen)
   */
  ...
  public BreakpointManager withOrientation() {
    ...
    heightListener =
        (o, old, h) -> updateOrientation(stage.getScene(), stage.getWidth(), h.doubleValue());
    stage.heightProperty().addListener(heightListener);
    return this;
  }

  /** Currently active breakpoint */
  public Breakpoint current() {
    return current;
  }

  /**
   * Returns a read-only property of the active breakpoint. Subscribers can listen to this property
   * to react to breakpoint changes. This is the main mechanism for the Publisher-Subscriber
   * pattern.
   *
  ...
  public boolean is(Breakpoint bp) {
    return current.minWidth >= bp.minWidth;
  }

  /** If current width is < the given breakpoint (below) */
  public boolean below(Breakpoint bp) {
    return !is(bp);
  }
  ...
  private void updateDefault(double width) {
    ...
    if (scene != null && scene.getRoot() != null) applyDefaultClasses(scene, next);
  ...
  private void updateCustom(double width) {
    String next = null;
    for (CustomBreakpoint bp : customBps) if (width >= bp.minWidth) next = bp.cssClass;
    ...
    if (scene != null && scene.getRoot() != null) {
      ...
      root.getStyleClass().removeIf(c -> customBps.stream().anyMatch(bp -> bp.cssClass.equals(c)));
      for (CustomBreakpoint bp : customBps)
        if (width >= bp.minWidth && bp.cssClass != null) root.getStyleClass().add(bp.cssClass);
    }
  }
  ...
  private record BpListener(double minWidth, Runnable runnable, Consumer<Boolean> consumer) {
