 ...
 *
 * <p>Attaches CSS utility classes to a node based on the scene width, replacing them automatically
 * as the window resizes. Uses {@link UtilityConflictResolver} internally — transitions are
 * deterministic.
 *
 * <h3>Usage</h3>
 *
 ...
 *
 * <ol>
 *   <li>Listens to {@code scene.widthProperty()} (not Stage — works in all contexts).
 *   <li>On resize, finds the highest breakpoint whose minWidth ≤ scene width.
 *   <li>Removes the previous breakpoint's classes, applies the new ones via {@link
 *       UtilityConflictResolver#applyAll}.
 *   <li>Base classes (registered with {@link #base}) are always present and are applied before
...
public final class ResponsiveNode {
  ...
  public static final class Builder {
    ...
    /**
     * Classes to apply always (base state, width 0+). Applied first; breakpoint classes are layered
     * on top.
     */
    public Builder base(String... classes) {
      baseClasses = classes;
      return this;
    }

    /** Classes to apply at scene width >= 640px (Tailwind {@code sm:}). */
    public Builder sm(String... classes) {
      return at(640, classes);
    }

    /** Classes to apply at scene width >= 768px (Tailwind {@code md:}). */
    public Builder md(String... classes) {
      return at(768, classes);
    }

    /** Classes to apply at scene width >= 1024px (Tailwind {@code lg:}). */
    public Builder lg(String... classes) {
      return at(1024, classes);
    }

    /** Classes to apply at scene width >= 1280px (Tailwind {@code xl:}). */
    public Builder xl(String... classes) {
      return at(1280, classes);
    }

    /** Classes to apply at scene width >= 1536px (Tailwind {@code 2xl:}). */
    public Builder xxl(String... classes) {
      return at(1536, classes);
    }

    /**
     * Classes to apply at a custom minimum width (in px). Multiple calls with the same width
     * replace each other.
     *
    ...
    /**
     * Registers a callback invoked whenever the active breakpoint changes. Receives the new minimum
     * width (in px) of the active rule.
     *
    ...
    /**
     * Attaches responsive listeners to the given Scene and returns a {@link ResponsiveNode} handle
  ...
  private ResponsiveNode(
      Node node,
      TreeMap<Integer, String[]> rules,
      String[] baseClasses,
  ...
  /**
   * Detaches the responsive width listener from the scene. The listener is also removed
   * automatically when the node leaves the scene graph, so explicit calls to {@code detach()} are
   * only needed for early cleanup.
   */
  public void detach() {
  ...
  /**
   * Installs a scene listener that auto-detaches the width listener when the node is removed from
   * the scene graph.
   *
   * <p>This prevents the responsive node from keeping the scene alive through the width listener
   * after the node has been removed from the UI.
   */
  ...
  /**
   * Returns the currently active minimum width breakpoint, or {@code -1} if only base classes are
   * active.
   */
  ...
  /**
   * Forces re-evaluation of responsive rules at the current scene width. Useful after programmatic
   * changes to the node's class list.
   */
  public void refresh() {
    applyForWidth(scene.getWidth());
  }
