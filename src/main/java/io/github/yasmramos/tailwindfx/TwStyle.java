...
public final class TwStyle {
    ...
    private static final Set<String> LAYOUT_DEPENDENT_PREFIXES = new HashSet<>(Arrays.asList(
        "m-", "mx-", "my-", "mt-", "mr-", "mb-", "ml-", 
        "gap-", "gap-x-", "gap-y-",
        "flex-", "grow", "shrink"
    ));
    
    private static final Set<String> RESPONSIVE_PREFIXES = new HashSet<>(Arrays.asList(
        "sm:", "md:", "lg:", "xl:", "2xl:"
    ));
    
    private static final Set<String> STATE_PREFIXES = new HashSet<>(Arrays.asList(
    ...
    private static void applyInternal(Node node, String... tokens) {
            ...
            for (String t : token.split("\\s+")) {
                ...
                if (hasUnsupportedVariant(t) && isLayoutDependent(t)) {
                    throw new UnsupportedOperationException(
                        "Layout-dependent properties do not support responsive or state variants: " + t + 
        ...
        if (!layoutMigrationTokens.isEmpty()) {
            throw new UnsupportedOperationException(
                "Layout classes requiring container migration (" + String.join(", ", layoutMigrationTokens) + 
    ...
    /**
     * Applies layout-dependent styles that require parent context (margins, gaps, flex).
     * These cannot be handled by CSS alone in JavaFX.
     */
    ...
    /**
     * Gets the effective parent pane, handling special cases like FxFlexPane.
     */
     ...
     */
    private static void applySingleLayoutStyle(Node node, javafx.scene.layout.Pane parent, String token) {
        if (token.startsWith("m-") || token.startsWith("mx-") || token.startsWith("my-") ||
            token.startsWith("mt-") || token.startsWith("mr-") || token.startsWith("mb-") || 
            token.startsWith("ml-")) {
            // Delegate to Styles.java for margin handling
            applyMarginStyleViaStyles(node, token);
        } else if (token.startsWith("gap-") || token.startsWith("gap-x-") || token.startsWith("gap-y-")) {
    ...
    /**
     * Delegates margin application to Styles.java methods.
     * Supports both numeric values (m-4) and arbitrary values (m-[20px]).
     */
    ...
    /**
     * Delegates flex application to FxFlexPane or Styles.java methods.
     */
    private static void applyFlexStyleViaStyles(Node node, javafx.scene.layout.Pane parent, String token) {
        // Prioritize FxFlexPane if parent is FxFlexPane
        if (parent instanceof io.github.yasmramos.tailwindfx.components.FxFlexPane flexPane) {
            applyFlexForFxFlexPane(node, token);
            return;
        }
    ...
    /**
     * Applies flex styles specifically for FxFlexPane container.
     */
    private static void applyFlexForFxFlexPane(Node node, String token) {
        if (token.equals("grow") || token.equals("flex-1")) {
            io.github.yasmramos.tailwindfx.components.FxFlexPane.setGrow(node, 1);
        } else if (token.equals("shrink") || token.equals("flex-none")) {
            io.github.yasmramos.tailwindfx.components.FxFlexPane.setShrink(node, 0);
        } else if (token.equals("flex-auto")) {
            io.github.yasmramos.tailwindfx.components.FxFlexPane.setGrow(node, 1);
            io.github.yasmramos.tailwindfx.components.FxFlexPane.setShrink(node, 1);
        } else if (token.equals("flex-initial")) {
            io.github.yasmramos.tailwindfx.components.FxFlexPane.setGrow(node, 0);
            io.github.yasmramos.tailwindfx.components.FxFlexPane.setShrink(node, 1);
        } else if (token.startsWith("flex-")) {
            // Handle arbitrary flex values like flex-[2]
            try {
                ...
                double flexValue = Double.parseDouble(value);
                io.github.yasmramos.tailwindfx.components.FxFlexPane.setGrow(node, flexValue);
            } catch (NumberFormatException e) {
                // Ignore invalid flex values
            }
        }
    ...
    private static void applyGapStyle(javafx.scene.layout.Pane parent, String token) {
        ...
        // Prioritize FxGridPane if parent is FxGridPane
        if (parent instanceof io.github.yasmramos.tailwindfx.components.FxGridPane gridPane) {
            if (token.startsWith("gap-x-")) {
                gridPane.gapX(px);
            } else if (token.startsWith("gap-y-")) {
                gridPane.gapY(px);
    ...
    private static void registerLayoutListener(Node node, java.util.List<String> tokens) {
        ...
        wrapper.listener = (obs, oldParent, newParent) -> {
    ...
    private static boolean isJitToken(String token) {
        ...
        if (token.equals("grow") || token.equals("shrink") || 
            token.equals("flex-none") || token.equals("flex-auto") || 
            token.equals("flex-1")) {
            return true;
        }
