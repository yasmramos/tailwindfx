...
/**
 * Preconditions — Centralized validation for TailwindFX. Three levels: 1. requireNonNull() →
 * IllegalArgumentException (immediate failure) 2. requireValid() → IllegalArgumentException (out of
 * range value) 3. warnIf() → LOG WARNING (valid but suspicious)
 */
public final class Preconditions {
  ...
  public static <T> T requireNonNull(T value, String method, String paramName) {
    if (value == null) {
      throw new IllegalArgumentException(method + ": '" + paramName + "' cannot be null");
    }
    return value;
  }
  ...
  public static String requireNonBlank(String value, String method, String paramName) {
    ...
    if (value.isBlank()) {
      throw new IllegalArgumentException(method + ": '" + paramName + "' cannot be blank");
    }
    return value;
  }
  ...
  public static int requireSpan(int span, String method) {
    if (span < 1) {
      throw new IllegalArgumentException(method + ": span must be >= 1, got: " + span);
    }
    if (span > MAX_SPAN) {
  ...
  public static int requirePositiveDuration(int ms, String method) {
    if (ms <= 0) {
      throw new IllegalArgumentException(method + ": duration must be > 0 ms, got: " + ms);
    }
  ...
  public static double requirePositiveScale(double scale, String method) {
    if (scale <= 0) {
      throw new IllegalArgumentException(method + ": scale must be > 0, got: " + scale);
    }
    if (scale > MAX_SCALE) {
      throw new IllegalArgumentException(
  ...
  public static double requirePositiveSpeed(double speed, String method) {
    if (speed <= 0) {
      throw new IllegalArgumentException(method + ": speed must be > 0, got: " + speed);
    }
    if (speed > MAX_SPEED) {
      throw new IllegalArgumentException(
  ...
  public static void warnNoParent(javafx.scene.Node node, String method) {
    if (node.getParent() == null) {
      LOG.warning(
          method
              + ": node has no parent — constraint will have no effect until added to the scene graph");
    }
  }

  /**
   * Warns if brightness is out of the recommended range [0.0-2.0]. Values < 0.0 are invalid and
   * will throw; values > 2.0 trigger a warning only.
   */
  public static void warnBrightnessRange(double value, String method) {
    if (value < 0.0) {
      throw new IllegalArgumentException(method + ": brightness cannot be negative, got: " + value);
    }
    if (value > 2.0) {
      LOG.warning(
          String.format(
  ...
  /**
   * Warns if a JIT token looks like a known typo. Note: This is a heuristic warning; not all
   * unrecognized tokens are typos.
   */
