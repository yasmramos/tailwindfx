 ...
 *
 * <p>Provides a declarative API over Timeline + KeyFrame + Interpolator. Zero javafx.animation
 * imports in user code.
 *
 * <p>Usage: TwAnimation.fadeIn(node).play(); TwAnimation.fadeIn(node, 300).play();
 * TwAnimation.slideUp(node).onFinished(e -> ...).play();
 * TwAnimation.pulse(node).cycleCount(3).play(); TwAnimation.onHoverScale(button, 1.05);
 * TwAnimation.chain( TwAnimation.fadeIn(node), TwAnimation.slideUp(node, 200) ).play();
 */
public final class TwAnimation {
   ...
   *
   * <p>The node's initial opacity is set to 0. The animation uses {@code EASE_OUT} interpolation
   * for a smooth entrance effect.
   *
   ...
   *
   * <p>The node's initial opacity is set to 0. The animation applies the specified interpolator for
   * fine-grained control over acceleration/deceleration.
   *
   * <pre>
  ...
  public static TwAnimation fadeIn(Node node, int durationMs, Interpolator ease) {
    ...
    TwAnimation fxAnim =
        new TwAnimation(
            new Timeline(
                new KeyFrame(Duration.ZERO, new KeyValue(node.opacityProperty(), 0, ease)),
                new KeyFrame(
                    Duration.millis(durationMs), new KeyValue(node.opacityProperty(), 1, ease))));
    fxAnim.ease = ease;
   ...
   *
   * <p>The node starts 20 pixels below its current position with opacity 0, then slides up and
   * fades to full opacity. Uses {@code EASE_OUT} interpolation.
   *
   * @param node the node to animate (must not be null)
   ...
   *
   * <p>The node starts 20 pixels below its current position with opacity 0, then slides up to its
   * original position while fading to full opacity. Uses {@code EASE_OUT} interpolation for smooth
   * motion.
   *
   * @param node the node to animate (must not be null)
   * @param durationMs animation duration in milliseconds (must be > 0)
   ...
   *
   * <p>The node starts 20 pixels above its current position with opacity 0, then slides down and
   * fades to full opacity. Uses {@code EASE_OUT} interpolation.
   *
   * @param node the node to animate (must not be null)
   * @return a ready-to-play {@link TwAnimation}
   ...
   *
   * <p>The node starts 20 pixels above its current position with opacity 0, then slides down to its
   * original position while fading to full opacity. Uses {@code EASE_OUT} interpolation for smooth
   * motion.
   *
   * @param node the node to animate (must not be null)
   ...
   *
   * <p>The node starts 24 pixels to the left with opacity 0, then slides right and fades to full
   * opacity. Uses {@code EASE_OUT} interpolation.
   *
   * @param node the node to animate (must not be null)
   * @return a ready-to-play {@link TwAnimation}
   ...
   * @throws IllegalArgumentException if node is null
   */
  public static TwAnimation slideRight(Node node) {
    return slideRight(node, NORMAL);
  }

  /**
   * Slides a node from left to right while fading in with specified duration.
   *
   * <p>The node starts 24 pixels to the left with opacity 0, then slides right to its original
   * position while fading to full opacity. Uses {@code EASE_OUT} interpolation.
   *
   * @param node the node to animate (must not be null)
   * @param durationMs animation duration in milliseconds (must be > 0)
   * @return a ready-to-play {@link TwAnimation}
   ...
   *
   * <p>The node starts 24 pixels to the right with opacity 0, then slides left and fades to full
   * opacity. Uses {@code EASE_OUT} interpolation.
   *
   * @param node the node to animate (must not be null)
   * @return a ready-to-play {@link TwAnimation}
   * @throws IllegalArgumentException if node is null
   */
  public static TwAnimation slideLeft(Node node) {
    return slideInLeft(node, NORMAL);
  }

  /**
   * Slides a node in from the left with specified duration.
   *
   * <p>The node starts off-screen to the left with opacity 0, then slides in to its original
   * position while fading to full opacity. Uses {@code EASE_OUT} interpolation.
   *
   * @param node the node to animate (must not be null)
   * @param durationMs animation duration in milliseconds (must be > 0)
   ...
   *
   * <p>The node starts off-screen to the right with opacity 0, then slides in to its original
   * position while fading to full opacity. Uses {@code EASE_OUT} interpolation.
   *
   * @param node the node to animate (must not be null)
   * @param durationMs animation duration in milliseconds (must be > 0)
   ...
   *
   * <p>The node starts off-screen below with opacity 0, then slides in to its original position
   * while fading to full opacity. Uses {@code EASE_OUT} interpolation.
   *
   * @param node the node to animate (must not be null)
   * @param durationMs animation duration in milliseconds (must be > 0)
   * @return a ready-to-play {@link TwAnimation}
   ...
   *
   * <p>The node starts off-screen above with opacity 0, then slides in to its original position
   * while fading to full opacity. Uses {@code EASE_OUT} interpolation.
   *
   * @param node the node to animate (must not be null)
   * @param durationMs animation duration in milliseconds (must be > 0)
   ...
   *
   * <p>Creates a "pop-in" entrance effect with {@code EASE_OUT} interpolation. The node starts at
   * 85% scale with opacity 0.
   *
   * @param node the node to animate (must not be null)
   * @return a ready-to-play {@link TwAnimation}
   ...
   * @throws IllegalArgumentException if node is null
   */
  public static TwAnimation scaleIn(Node node) {
    return scaleIn(node, NORMAL);
  }
   ...
   *
   * <p>Creates a "pop-in" entrance effect. The node starts at 85% scale with opacity 0, then scales
   * to 100% and fades to full opacity using the specified interpolator.
   *
   * @param node the node to animate (must not be null)
   * @param durationMs animation duration in milliseconds (must be > 0)
   * @param ease the interpolation function (must not be null)
   ...
   * @throws IllegalArgumentException if node is null
   */
  public static TwAnimation fadeOut(Node node) {
   ...
   *
   * <p>Uses {@code EASE_IN} interpolation for a smooth exit effect. Automatically registered in the
   * {@code "exit"} slot, cancelling any previous exit animation.
   *
   * @param node the node to animate (must not be null)
   * @param durationMs animation duration in milliseconds (must be > 0)
   * @return a ready-to-play {@link TwAnimation} registered in slot {@code "exit"}
   * @see #fadeOut(Node)
   * @throws IllegalArgumentException if node is null or durationMs <= 0 (calls
  ...
  public static TwAnimation fadeOut(Node node, int durationMs) {
    ...
    return new TwAnimation(
            new Timeline(
                new KeyFrame(
                    Duration.ZERO, new KeyValue(node.opacityProperty(), node.getOpacity())),
                new KeyFrame(
                    Duration.millis(durationMs),
   ...
   * @throws IllegalArgumentException if node is null
   */
  public static TwAnimation scaleOut(Node node) {
    return scaleOut(node, NORMAL);
  }

  /**
   * Scales a node down from 1.0 to 0.85 while fading out with specified duration.
   *
   * <p>Creates a "pop-out" exit effect. The node scales down to 85% and fades to opacity 0 using
   * {@code EASE_IN} interpolation.
   *
   * @param node the node to animate (must not be null)
   * @param durationMs animation duration in milliseconds (must be > 0)
   * @return a ready-to-play {@link TwAnimation}
   ...
   *
   * <p>Useful for drawing attention to badges, notifications, or status indicators. The animation
   * cycles with period of 1 second (500ms fade out, 500ms fade in). Call {@link
   * TwAnimation#cycleCount(int)} to limit the number of pulses.
   *
   * @param node the node to animate (must not be null)
  ...
  public static TwAnimation shake(Node node) {
    ...
    return new TwAnimation(
        new Timeline(
            ...
            new KeyFrame(
                Duration.millis(60),
                new KeyValue(node.translateXProperty(), x - 8, Interpolator.EASE_BOTH)),
            new KeyFrame(
                Duration.millis(120),
                new KeyValue(node.translateXProperty(), x + 8, Interpolator.EASE_BOTH)),
            new KeyFrame(
                Duration.millis(180),
                new KeyValue(node.translateXProperty(), x - 6, Interpolator.EASE_BOTH)),
            new KeyFrame(
                Duration.millis(240),
                new KeyValue(node.translateXProperty(), x + 6, Interpolator.EASE_BOTH)),
            new KeyFrame(
                Duration.millis(300),
                new KeyValue(node.translateXProperty(), x - 4, Interpolator.EASE_BOTH)),
            new KeyFrame(
                Duration.millis(360),
                new KeyValue(node.translateXProperty(), x, Interpolator.EASE_OUT))));
  }

  /**
   * Bounces a node vertically with a dampening effect.
   *
   * <p>Useful for attention-drawing animations on errors or interactive feedback. The node bounces
   * up and down with decreasing amplitude over 480 milliseconds.
   *
   * @param node the node to animate (must not be null)
   * @return a ready-to-play {@link TwAnimation}
   * @throws IllegalArgumentException if node is null
  ...
  public static TwAnimation bounce(Node node) {
    ...
    return new TwAnimation(
        new Timeline(
            ...
            new KeyFrame(
                Duration.millis(100),
                new KeyValue(node.translateYProperty(), y - 12, Interpolator.EASE_OUT)),
            new KeyFrame(
                Duration.millis(200),
                new KeyValue(node.translateYProperty(), y, Interpolator.EASE_IN)),
            new KeyFrame(
                Duration.millis(280),
                new KeyValue(node.translateYProperty(), y - 6, Interpolator.EASE_OUT)),
            new KeyFrame(
                Duration.millis(360),
                new KeyValue(node.translateYProperty(), y, Interpolator.EASE_IN)),
            new KeyFrame(
                Duration.millis(420),
                new KeyValue(node.translateYProperty(), y - 3, Interpolator.EASE_OUT)),
            new KeyFrame(
                Duration.millis(480),
                new KeyValue(node.translateYProperty(), y, Interpolator.EASE_IN))));
  }
   ...
   *
   * <p>Useful for toast notifications or quick attention-grabbing effects. The animation completes
   * a full flash cycle in 800 milliseconds (4 toggles).
   *
   * @param node the node to animate (must not be null)
   * @return a ready-to-play {@link TwAnimation}
   * @throws IllegalArgumentException if node is null
   ...
   *
   * <p>Perfect for loading spinners, progress indicators, or hover effects. Uses default duration
   * of 1000ms per full rotation with {@code LINEAR} interpolation.
   *
   * @param node the node to animate (must not be null)
   * @return a ready-to-play {@link TwAnimation} with infinite cycle count
   * @see #spin(Node, int)
   ...
   *
   * <p>Perfect for loading spinners or progress indicators. Rotates 360 degrees in the specified
   * duration using {@code LINEAR} interpolation.
   *
   * @param node the node to animate (must not be null)
   ...
   *
   * <p>Useful for status indicators, pulsing notifications, or meditative UI elements. The node
   * scales from 1.0 to 1.06 (6% growth) with fading opacity over a 2-second cycle, creating a
    ...
    Timeline tl =
        new Timeline(
            ...
            new KeyFrame(
                ...
                new KeyValue(node.scaleXProperty(), 1.06),
                new KeyValue(node.scaleYProperty(), 1.06),
                new KeyValue(node.opacityProperty(), 0.85)),
            new KeyFrame(
  ...
  public static void onHoverScale(Node node, double scaleFactor) {
    ...
    node.addEventHandler(
        ...
        e -> animateScale(node, baseScaleX * scaleFactor, baseScaleY * scaleFactor, 150));
    node.addEventHandler(
        javafx.scene.input.MouseEvent.MOUSE_EXITED,
        e -> animateScale(node, baseScaleX, baseScaleY, 150));
  }

  /**
   * Internal helper that animates scale transitions smoothly.
   *
   * <p>Used by hover effects to animate between scale values. Supports smooth interpolation from
   * current scale to target scale.
   *
   * @param node the node to animate (must not be null)
   ...
   *
   * <p>Each animation waits for the previous one to complete. Useful for choreographed entrance
   * sequences or multi-step effects.
   *
   * <pre>
   * TwAnimation.chain(
   ...
   *
   * <p>All animations play at the same time. The overall animation completes when the longest
   * animation finishes. Useful for combined effects on the same or different nodes.
   *
   ...
   *
   * <p>Use {@link Animation#INDEFINITE} for infinite looping. By default, animations play once
   * (cycle count = 1).
   *
   ...
   *
   * <p>Convenience method equivalent to {@code cycleCount(Animation.INDEFINITE)}. Stop explicitly
   * with {@link #stop()} or when the node is removed from the scene.
   *
   * @return this animation for method chaining
   * @see #cycleCount(int)
   */
   ...
   *
   * <p>Values less than 1.0 slow down the animation; values greater than 1.0 speed it up. For
   ...
   *
   * <p>Useful for animations that should return to their starting state automatically (e.g., hover
   * effects, attention animations). Only has effect when cycle count is > 1.
   *
   * @return this animation for method chaining
   */
   ...
   *
   * <p>When {@link #play()} is called, any previous animation in the same slot is automatically
   * cancelled. This prevents competing animations from fighting over the same node properties.
   *
   * <p>Standard slots:
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>Custom slots can be used for independent animations that should not cancel each other.
   *
   ...
   *
   * <p>If this animation is registered, any previous animation in the same slot is automatically
   * cancelled before this one starts. Otherwise, plays the animation directly on its underlying
   * Timeline.
   *
   ...
   *
   * <p>Use the convenience shortcuts {@link #easeIn()}, {@link #easeOut()}, {@link #easeBoth()},
   * {@link #linear()} for common cases.
   *
   ...
   * @see #easeBoth()
   */
  public TwAnimation easeIn() {
    return withEase(Interpolator.EASE_IN);
  }

  /**
   * Applies {@code EASE_OUT} interpolation — decelerates at the end.
   *
   * <p>Use for animations that should start fast and end slow (e.g., entrances). This is the most
   * common easing for natural-feeling animations.
   *
   * @return this animation for method chaining
   * @see #easeIn()
   * @see #easeBoth()
   */
  public TwAnimation easeOut() {
    return withEase(Interpolator.EASE_OUT);
  }
   ...
   * @see #easeOut()
   */
  public TwAnimation easeBoth() {
    return withEase(Interpolator.EASE_BOTH);
  }
   ...
   * @see #easeOut()
   */
  public TwAnimation linear() {
    return withEase(Interpolator.LINEAR);
  }

  /**
   * Returns the underlying JavaFX {@link Animation} for advanced configuration.
   *
   * <p>Use this for direct access to features not exposed by TwAnimation's fluent API. Most users
   * should use the fluent API methods instead.
   *
   * @return the underlying Timeline or Transition
   */
  public Animation raw() {
    return timeline;
  }
   ...
   *
   * <p>Installs permanent mouse enter/exit listeners. Multiple hover effects can be stacked on the
   * same node. Remove all effects with {@link #removeHoverEffects(Node)}.
   *
   ...
   */
  public static void onHoverLift(Node node) {
    onHoverLift(node, -4);
  }
   ...
   *
   * <p>Installs permanent mouse enter/exit listeners that animate the node up/down. Negative
   * offsets lift the node up; positive offsets push it down. Multiple hover effects can be stacked.
   * Remove all with {@link #removeHoverEffects(Node)}.
   *
   * @param node the node to add lift effect to (must not be null)
  ...
  public static void onHoverLift(Node node, double offsetY) {
    ...
    Timeline hoverIn =
        new Timeline(
            new KeyFrame(
                Duration.millis(FAST),
                new KeyValue(
                    node.translateYProperty(), originalY + offsetY, Interpolator.EASE_OUT)));
    Timeline hoverOut =
        new Timeline(
            new KeyFrame(
                Duration.millis(FAST),
   ...
   *
   * <p>Unregisters all mouse listeners added by {@link #onHoverScale(Node, double)}, {@link
   * #onHoverLift(Node)}, {@link #onHoverLift(Node, double)}, and {@link #onHoverDim(Node, double)}.
   * Also resets scale, opacity, and rotation to baseline values (1.0, 1.0, 0).
   *
  ...
  public static void removeHoverEffects(Node node) {
    ...
    var handlers =
        (java.util.List<javafx.event.EventHandler<javafx.scene.input.MouseEvent>>)
   ...
   *
   * <p>Maintains a list of all hover handlers installed on a node so they can be unregistered
   * cleanly by {@link #removeHoverEffects(Node)}.
   *
   * @param node the target node (must not be null)
  ...
  private static void storeHoverHandlers(
    ...
    var handlers =
        (java.util.List<javafx.event.EventHandler<javafx.scene.input.MouseEvent>>)
            node.getProperties()
                .computeIfAbsent("tailwindfx.hover.handlers", k -> new java.util.ArrayList<>());
    handlers.add(enter);
    handlers.add(exit);
  }
   ...
   *
   * <p>Registers animations in named slots, auto-cancels previous animations in the same slot, and
   * automatically cleans up when nodes are removed from the scene. This prevents Timeline objects
   * from holding strong references to detached nodes.
   *
   * <h3>Predefined Slots</h3>
   *
   * Each slot governs a semantic category of animation. Starting an animation in a slot cancels
   * whatever is currently running in that slot:
   *
   * <ul>
   ...
   * <h3>Custom Slots</h3>
   *
   * Any string is a valid slot. Use custom slots when you need independent animations that should
   ...
   *
   * Automatically removes all active animations when the node is detached from the scene,
   * preventing Timeline memory leaks.
   */
  public static final class AnimationRegistry {
     ...
     *
     * <p>If an animation is already running in the same slot, it is automatically stopped and
     * replaced. Also installs a scene listener that auto-cancels all animations if the node is
     * removed from the scene, preventing Timeline-induced memory leaks.
     *
     * @param node the target node (must not be null)
    ...
    /**
     * Installs a scene listener (once per node) that cancels all active animations when the node is
     * removed from the scene graph.
     *
     * <p><b>Why this matters:</b> A running Timeline holds references to KeyValues, which hold
     * references to node properties, creating a chain: Timeline → KeyValue → node. This prevents GC
     * of detached nodes. Cancelling the Timeline on scene removal breaks this chain.
     *
     * @param node the target node (must not be null)
     */
    ...
    private static java.util.Map<String, Animation> getSlots(javafx.scene.Node node) {
      return (java.util.Map<String, Animation>)
          node.getProperties()
   ...
   *
   * <p>Prevents animations from becoming desynchronized when the layout reorganizes (e.g., when
   * crossing a breakpoint that changes from row to column layout).
   *
   * <h3>Problem Scenario</h3>
   *
   * If a node is animating its translateX/Y while the layout simultaneously recalculates positions,
   * the node can end up in an incorrect final position.
   *
   * <h3>Solution</h3>
   *
   * Pause active transform animations before layout changes, then resume afterward. This
   * integration with BreakpointManager ensures smooth responsive behavior.
   *
   * <pre>
  ...
  public static final class ResponsiveAnimationGuard {
     ...
     *
     * <p>Call at the start of a responsive layout change (before reorganizing Panes). Animations in
     * transform slots (enter, exit, slide, scale) are paused to prevent conflicting with position
     * recalculations.
     *
     ...
     *
     * <p>Call after completing the layout reorganization. Animations that were active before the
     * layout change are automatically resumed.
     *
     ...
     *
     * <p>Cancels all active animations and restores the node to its default visual state:
     * translateX/Y = 0, scaleX/Y = 1, opacity = 1, rotation = 0. Useful when switching layouts to
     * prevent residual transform values from affecting the new layout.
     *
    ...
    private static void pauseAnimationsInSubtree(javafx.scene.Node node) {
      @SuppressWarnings("unchecked")
      var slots =
          (java.util.Map<String, Animation>) node.getProperties().get("tailwindfx.animations");
      if (slots != null) {
     ...
     *
     * <p>Slots like "enter", "exit", "slide*", "scale*" animate position and size. These must be
     * paused during responsive layout changes.
     *
