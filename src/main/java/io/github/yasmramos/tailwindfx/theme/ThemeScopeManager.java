 ...
 *
 * <p>Problema: ThemeManager es global (afecta toda la Scene). Para apps enterprise con secciones de
 * tema diferente en la misma pantalla: - Panel de admin: dark - Embedded form: light - Sidebar:
 * custom accent
 *
 * <p>Solution: apply Modena variables as inline style on any Pane, which JavaFX propagates to all
 * its children (local CSS cascade).
 *
 * <p>Usage: // Entire scene in dark TailwindFX.theme(scene).dark().apply();
 *
 * <p>// Only the alerts panel with red accent ThemeScopeManager.scope(alertPanel)
 * .accent("#ef4444") .focus("#f87171") .apply();
 *
 * <p>// Partial override — only changes the background of a drawer ThemeScopeManager.scope(drawer)
 * .base("#1e293b") .apply();
 *
 * <p>// Clear scope from a node (inherits from parent) ThemeScopeManager.clearScope(drawer);
 *
 * <p>// Read what scope a node has Optional<String> base = ThemeScopeManager.getVar(myPane,
 * "-fx-base");
 */
public final class ThemeScopeManager {
   ...
   *
   * <p>TailwindFX also exposes this as: TailwindFX.scope(pane).dark().apply();
   */
  ...
  /**
   * Reads the value of a theme variable in a Pane's scope. Returns empty if the Pane does not have
   * its own scope for that variable.
   */
  ...
  /**
   * Copies the scope from one Pane to another (useful for modals that must inherit the theme from
   * their origin).
   */
  public static void copyScope(Pane source, Pane target) {
    ...
    props.forEach(
        (k, v) -> {
          if (isThemeVar(k)) themeProps.put(k, v);
        });
    ...
    public ScopeBuilder preset(String name) {
      return switch (name.toLowerCase()) {
        ...
        case "blue" -> base("#dbeafe")
            .accent("#2563eb")
            .focus("#3b82f6")
            .faintFocus("rgba(59,130,246,0.2)");
        case "green" -> base("#dcfce7")
            .accent("#16a34a")
            .focus("#22c55e")
            .faintFocus("rgba(34,197,94,0.2)");
        case "purple" -> base("#ede9fe")
            .accent("#7c3aed")
            .focus("#8b5cf6")
            .faintFocus("rgba(139,92,246,0.2)");
        case "rose" -> base("#ffe4e6")
            .accent("#e11d48")
            .focus("#f43f5e")
            .faintFocus("rgba(244,63,94,0.2)");
        case "slate" -> base("#e2e8f0")
            .accent("#475569")
            .focus("#64748b")
            .faintFocus("rgba(100,116,139,0.2)");
        default -> throw new IllegalArgumentException("Preset desconocido: " + name);
      };
    }

    // --- Variables individuales (override parcial) ---
    public ScopeBuilder base(String color) {
      vars.put("-fx-base", color);
      return this;
    }

    public ScopeBuilder background(String color) {
      vars.put("-fx-background", color);
      return this;
    }

    public ScopeBuilder innerBackground(String color) {
      vars.put("-fx-control-inner-background", color);
      return this;
    }

    public ScopeBuilder accent(String color) {
      vars.put("-fx-accent", color);
      vars.put("-fx-selection-bar", color);
      return this;
    }

    public ScopeBuilder focus(String color) {
      vars.put("-fx-focus-color", color);
      return this;
    }

    public ScopeBuilder faintFocus(String color) {
      vars.put("-fx-faint-focus-color", color);
      return this;
    }

    public ScopeBuilder defaultButton(String color) {
      vars.put("-fx-default-button", color);
      return this;
    }

    public ScopeBuilder selectionBar(String color) {
      vars.put("-fx-selection-bar", color);
      return this;
    }

    public ScopeBuilder cellHover(String color) {
      vars.put("-fx-cell-hover-color", color);
      return this;
    }

    public ScopeBuilder textFill(String color) {
      vars.put("-fx-text-base-color", color);
      return this;
    }

    /**
     * Aplica el scope al Pane. Merge no destructivo: preserva inline styles previos no relacionados
     * con theming.
     */
    public void apply() {
   ...
   *
   * <p>Useful when a node needs to know which scope it inherits. Returns {@code null} if no scoped
   * ancestor exists.
   *
   ...
   *
   * <p>Unlike {@link #copyScope(Pane, Pane)} which copies from a specific source, this method walks
   * up the scene graph from {@code node} to find the nearest scoped ancestor and copies its
   * variables to {@code target}. This is the correct way to give a modal or popover the same theme
   * as its trigger:
   *
   * <pre>
   ...
   *
   * <p>JavaFX does not fire style-related events on reparenting, so scoped inline styles are
   * preserved, but this method forces a refresh so that the new parent's CSS cascade takes effect
   * alongside the scope variables.
   *
   * <pre>
  ...
  private static final Set<String> THEME_VARS =
      Set.of(
          "-fx-base",
          "-fx-background",
