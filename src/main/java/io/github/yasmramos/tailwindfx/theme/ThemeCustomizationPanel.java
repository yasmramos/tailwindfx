 ...
 * - Cambiar colores primarios/secundarios dinámicamente
 * - Ajustar valores de tema (radius, spacing, etc.)
 * - Ver preview en tiempo real
 * - Guardar/cargar configuraciones personalizadas
 * - Exportar configuración como archivo JSON/properties
 * 
 * @author yasmramos
 * @version 0.1.1
 */
public class ThemeCustomizationPanel extends VBox {
    ...
    private VBox createPropertiesSection() {
        ...
        // Border Radius
        VBox radiusBox = createSliderControl(
            "Border Radius:",
            0, 24, 8,
            "px"
        );
        ...
        // Spacing Scale
        VBox spacingBox = createSliderControl(
            "Spacing Scale:",
            0.5, 2.0, 1.0,
            "x"
        );
        ...
        // Shadow Intensity
        VBox shadowBox = createSliderControl(
            "Shadow Intensity:",
            0, 100, 50,
            "%"
        );
     ...
     */
    private VBox createSliderControl(String label, double min, double max, double value, String unit) {
    ...
    private void applyLiveChanges() {
        try {
            ...
            previewBox.setStyle(String.format("""
            ...
            """, 
                toCssColor(bg), radius,
                toCssColor(primary), radius,
                shadow / 100.0, shadow / 5.0
            ));
    ...
    public void loadSavedConfiguration() {
        try {
            if (prefs.getDouble("primary.r", -1) != -1) {
                primaryColorPicker.setValue(new Color(
                ...
                secondaryColorPicker.setValue(new Color(
                ...
                backgroundColorPicker.setValue(new Color(
                ...
                textColorPicker.setValue(new Color(
    ...
    public void exportToFile() {
        ...
        if (file != null) {
            try (FileWriter writer = new FileWriter(file)) {
                ...
                writer.write(String.format("primary.color=%s\n", toCssColor(primaryColorPicker.getValue())));
                writer.write(String.format("secondary.color=%s\n", toCssColor(secondaryColorPicker.getValue())));
                writer.write(String.format("background.color=%s\n", toCssColor(backgroundColorPicker.getValue())));
